/* Gaming Zone v5 - Full Cleanup (No Product Icons) */
...
function endShift(){
    ...
    setTimeout(()=>{
        window.print();
        setTimeout(()=>{
            alert('✅ تم إنهاء الوردية وطباعة الفاتورة المدمجة');
            logout();
        },1000);
    },500);
}
...
function buildAccountingInvoice(transactions,total,shiftName){
    ...
    html+='<div class="acc-header"><div class="acc-logo">🎮</div><h1>GAMING ZONE</h1><p>نظام إدارة صالة الألعاب</p></div>';
    html+='<div class="acc-title">📋 فاتورة محاسبة الوردية (صفحة واحدة) 📋</div>';
    ...
    html+='<div class="acc-meta-row"><span>👑 <strong>المستلم:</strong></span><span>'+ownerName+'</span></div>';
    html+='<div class="acc-meta-row"><span>📅 <strong>التاريخ:</strong></span><span>'+dateStr+'</span></div>';
    html+='<div class="acc-meta-row"><span>🕐 <strong>الوقت:</strong></span><span>'+timeStr+'</span></div>';
    html+='<div class="acc-meta-row"><span>⏰ <strong>الوردية:</strong></span><span>'+shiftName+'</span></div>';
    html+='<div class="acc-meta-row"><span>📊 <strong>العمليات:</strong></span><span>'+realTransactions.length+'</span></div>';
    html+='</div>';
    
    html+='<div class="acc-section-title">💰 ملخص المداخيل حسب النوع</div>';
    html+='<table class="acc-table"><thead><tr><th>النوع</th><th style="text-align:center">العدد</th><th style="text-align:left">المبلغ</th></tr></thead><tbody>';
    ...
    if(realTransactions.length>0){
        html+='<div class="acc-section-title">📋 سجل العمليات</div>';
        ...
        realTransactions.forEach(t=>{
            const short=t.details.length>60?t.details.substring(0,60)+'...':t.details;
            html+='<tr><td>'+t.time+'</td><td>'+(types[t.type]||t.type)+'</td><td style="font-size:9px">'+short+'</td><td style="text-align:left" class="num">'+t.amount+' دج</td></tr>';
        });
        html+='</tbody></table>';
    }
    
    html+='<div class="acc-summary">';
    html+='<div class="acc-sum-row"><span>💰 مجموع مداخيل الأجهزة:</span><span>'+(pcTotal+consTotal)+' دج</span></div>';
    html+='<div class="acc-sum-row"><span>🛒 مجموع مداخيل السلع:</span><span>'+prodTotal+' دج</span></div>';
    html+='<div class="acc-grand"><span>💵 الإجمالي للتسليم:</span><span>'+total+' دج</span></div>';
    html+='</div>';
    ...
    html+='</div>';
    html+='</div><div class="acc-footer"><p class="thanks">🎮 GAMING ZONE 🎮</p></div></div>';
...
function renderPC(){
    ...
    for(let i=1;i<=d.pcCount;i++){
        ...
        h+='<span class="pc-status '+(a?'busy':'free')+'">'+(a?'🟢 في اللعب':'⚫ شاغر')+'</span></div>';
        h+='<div class="pc-card-body">';
        if(a){
            h+='<div class="pc-timer-display" id="pct-'+i+'">'+String(hr).padStart(2,'0')+':'+String(mn).padStart(2,'0')+'</div>';
            h+='<div class="pc-price-info">الساعة = <strong>'+d.pcPricePerHour+' دج</strong> | الوقت: <strong>'+pcCost+' دج</strong></div>';
            h+='<div style="font-weight:600;margin-top:5px">🛒 سلع زبون:</div><div class="consumption-list">';
            h+=co.length?co.map(c=>'<div class="consumption-item"><span>'+c.name+' ×'+c.qty+'</span><span>'+c.price*c.qty+' دج</span></div>').join(''):'<p class="empty-msg" style="padding:3px">لا توجد سلع</p>';
            h+='</div><div class="pc-total"><span>المجموع:</span><span class="pc-total-amount">'+(pcCost+ct)+' دج</span></div>';
        }else{
            h+='<p class="empty-msg">الجهاز شاغر ومتاح للعب</p>';
        }
        h+='</div><div class="pc-card-actions">';
        if(a){
            h+='<button class="btn-sm btn-add" onclick="openConModal(\'pc\','+i+')"><i class="fas fa-plus"></i> إضافة سلع</button>';
            h+='<button class="btn-sm btn-checkout" onclick="openChkModal(\'pc\','+i+')"><i class="fas fa-receipt"></i> حساب</button>';
        }else{
            h+='<button class="btn-sm btn-add" onclick="startPC('+i+')"><i class="fas fa-play"></i> تشغيل</button>';
        }
        h+='</div></div>';
    }
    g.innerHTML=h;
...
function renderCon(){
    ...
    d.consoles.forEach(c=>{
        ...
        if(a){
            ...
            h+='<div class="pc-total"><span>المجموع الكلي:</span><span class="pc-total-amount">'+(tt+ct)+' دج</span></div></div>';
            h+='<div class="pc-card-actions">';
            h+='<button class="btn-sm btn-add" onclick="openConModal(\'console\',\''+c.id+'\')"><i class="fas fa-plus"></i> إضافة سلع</button>';
...
function rmConTime(id){
    ...
    if(s&&s.remainingMinutes>=30){s.remainingMinutes-=30;s.totalHalves=Math.max(0,s.totalHalves-1);saveData(d);renderCon();}
}
...
/* ===== ADD PRODUCTS TO ACTIVE SESSION ===== */
...
function addMC(id){
    ...
    if(!p||p.stock<=0){alert('السلعة نفدت من المخزن!');return;}
    const e=modalCart.find(x=>x.id===id);
    if(e){if(e.qty<p.stock)e.qty++;else alert('المخزون المتوفر لا يكفي');}
...
function saveSessionCon(){
    ...
    if(!s){alert('العملية غير موجودة');return;}
    ...
    alert('✅ تم حفظ وإضافة السلع: '+total+' دج');
}
...
function openChkModal(type,id){
    ...
    if(type==='pc'){
        ...
        tl='وقت اللعب ('+(s.totalHalves||0)+' × نصف ساعة)';
    }
    ...
    else{html+='<span style="color:var(--text-muted)">لا توجد سلع مضافة</span>';}
    html+='</div>';
    ...
    html+='<div style="display:flex;justify-content:space-between;font-size:20px;font-weight:900;margin-top:12px;padding-top:10px;border-top:1px solid var(--border-color)"><span>💰 مجموع الفاتورة المطلوب:</span><span style="color:var(--neon-green)">'+(tt+ct)+' دج</span></div>';
...
function confirmChk(){
    ...
    alert('✅ تم تأكيد الدفع وإفراغ الجهاز');
}
...
function addQS(id){
    const d=getData(),p=d.products.find(x=>x.id===id);if(!p||p.stock<=0){alert('غير متوفر');return;}
...
function renderQSC(){
    ...
    if(!qsCart.length){c.innerHTML='<p class="empty-msg">اختر سلعاً من القائمة</p>';t.textContent='0 دج';return;}
...
function updateOv(){
    ...
    if(bx){
        ...
        bx.innerHTML=low.length?low.map(p=>'<div style="background:rgba(255,136,0,0.1);border:1px solid rgba(255,136,0,0.3);padding:6px 10px;border-radius:6px;margin-bottom:4px;font-size:12px">⚠️ السلعة: <strong>'+p.name+'</strong> قريبة من النفاد - المتبقي: <strong style="color:var(--neon-orange)">'+p.stock+'</strong></div>').join(''):'<p class="empty-msg" style="padding:4px">✅ المخزون ممتاز ولا توجد نواقص</p>';
    }
}
...
function renderCharts(){
    ...
    if(pCtx){
        pieC=new Chart(pCtx,{
            type:'doughnut',
            data:{labels:['أجهزة PC','الكونسول','السلع والحلويات'],datasets:[{data:[pc||1,co||1,pd||1],backgroundColor:['#00d4ff','#7b2ffa','#ff8800'],borderWidth:0}]},
    ...
    if(bCtx){
        barC=new Chart(bCtx,{
            type:'bar',
            data:{labels:['أجهزة PC','الكونسول','السلع'],datasets:[{label:'دج',data:[pc,co,pd],backgroundColor:['#00d4ff','#7b2ffa','#ff8800'],borderRadius:8}]},
...
function genReport(){
    ...
    if(lg){
        const tp={pc_time:'🖥️ وقت PC',console_time:'🎮 وقت كونسول',product_sale:'🛒 سلع',shift_end:'🚪 نهاية وردية'};
